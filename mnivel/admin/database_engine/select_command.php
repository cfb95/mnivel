...
class FieldAggregationInfo
{
    ...
    public function __construct($aggregate, $alias = '')
    {
...
class AggregationValuesQuery
{
    ...
    public function AddAggregate($fieldName, $aggregate, $alias = '')
    {
    ...
    private function GetFieldListAsSQL()
    {
        ...
        foreach($this->fieldAggregations as $fieldName => $aggregationInfo)
            if (!$aggregationInfo->HasAlias())
                StringUtils::AddStr($result, $this->GetAggregateAsSQL($fieldName, $aggregationInfo->GetAggregate()), ', ');
            else
                StringUtils::AddStr($result,
                    $this->engCommandImp->GetAliasedAsFieldExpression(
                        $this->GetAggregateAsSQL($fieldName, $aggregationInfo->GetAggregate()),
                        $aggregationInfo->GetAlias()
                    ), ', ');
        return $result;
    }

    private function GetAggregateAsSQL($fieldName, $aggregate)
    {
        return $this->engCommandImp->GetAggregationExpression($aggregate, $fieldName);
    }
