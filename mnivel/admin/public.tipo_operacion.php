 ...
 */
    ...
    class public_tipo_operacionPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('public_tipo_operacionssearch', $this->dataset,
                array('id', 'sigla', 'tipo_operacion'),
                array($this->RenderText('Id'), $this->RenderText('Sigla'), $this->RenderText('Tipo Operacion')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('public_tipo_operacionasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('tipo_operacion', $this->RenderText('Tipo Operacion')));
        }
        ...
        protected function AddFieldColumns($grid)
        {
            ...
            //
            $column = new TextViewColumn('tipo_operacion', 'Tipo Operacion', $this->dataset);
            $column->SetOrderable(true);
            ...
            //
            $editor = new TextEdit('tipo_operacion_edit');
            $editor->SetSize(64);
            $editColumn = new CustomEditColumn('Tipo Operacion', 'tipo_operacion', $editor, $this->dataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $editColumn->GetCaption()));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $column->SetEditOperationColumn($editColumn);
        ...
        public function GetModalGridDeleteHandler() { return 'public_tipo_operacion_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'public_tipo_operacionGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new public_tipo_operacionPage("public.tipo_operacion.php", "public_tipo_operacion", GetCurrentUserGrantForDataSource("public_tipo_operacion"), 'UTF-8');
        $Page->SetShortCaption('Tipos de Operaciones');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Tipos de Operaciones');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("public_tipo_operacion"));
